"""Built-in LLM-judge evaluation metrics."""
...
class BuiltinEvaluator:
    ...
    def evaluate(self, config: EvalConfig) -> EvalResult:
        ...
        for q in config.questions:
            per_question.append(self._evaluate_question(rag, q, config.metrics))
    ...
    def _evaluate_question(
        ...
        try:
            result: RAGResult = rag.query(q.question)
        ...
        if "faithfulness" in metrics:
            ...
            row["faithfulness"] = self._llm_judge_faithfulness(rag, result.answer, context)

        if "answer_similarity" in metrics:
            row["answer_similarity"] = self._llm_judge_similarity(
                rag, q.expected_answer, result.answer
            )
    ...
    def _llm_judge_faithfulness(self, rag: RAGPipeline, answer: str, context: str) -> float:
        from langchain_core.messages import HumanMessage

        prompt = FAITHFULNESS_PROMPT.format(context=context[:3000], answer=answer)
        try:
            response = rag._llm.invoke([HumanMessage(content=prompt)])
            score = float(response.content.strip())
            return max(0.0, min(1.0, score))
        except (ValueError, AttributeError):
            return 0.0

    def _llm_judge_similarity(self, rag: RAGPipeline, expected: str, actual: str) -> float:
        from langchain_core.messages import HumanMessage
